...
/**
 * The packet sent to a client from a BBServer instance in response to the client's sending a {@link Packet0AuthRequest}
 * to attempt authorization in order to join the server. This packet contains the server's verdict, and if the user was
 * successfully authorized, the server will now wait for the client to send a {@link Packet2PlayerJoin} to begin
 * interaction with the server.<p>
 * This packet is only ever received (never sent) by the client.
 * 
 * @author LinearLogic
 * @since 0.2.5
 */
